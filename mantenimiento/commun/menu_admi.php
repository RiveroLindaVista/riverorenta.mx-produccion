<?php
  ...
  $consultas=new Consultas();
    ...
    $notif=$consultas->get_admin_notificaciones();
...
?>
    ...
    <script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp > notif_actual){
                        ...
                        $("#badgeN").html('<a class="nav-link" href="#" style="position:relative;"><span><i class="fa fa-bell text-white"  aria-hidden="true"></i></span><span id="badgeRed" class="badge" style="top:0;right:0; margin-top: -5px;position:absolute;background-color:red;color:white;box-shadow: 2px 2px 5px gray;">'+resp+'</span></a>');
                    }
                }
            });
        }, 10000);
            ...
            $.ajax({
                ...
                success: function (resp) {
                    for(let i=0;i<resp.length;i++){
                        $not += "<div class='p-3' style='background-color:#E3E9FB;border: 1px solid #4573F8;cursor:pointer;' onclick='irNotificacion("+resp[i]["id"]+")'>#"+resp[i]["id"]+" - "+resp[i]["log_sistema"]+" <i class='fa fa-share' aria-hidden='true'></i> </div>";
                    }
        ...
        function irNotificacion(id){
            ...
            let params = {
                id: id,
                func: "limpiar_notificacion_sistema"
            };
